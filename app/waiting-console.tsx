  ...
  const moveTicket = async (ticketNumber: number, status: TicketStatus) => {
    ...
    if (error || !data?.ok) {
      const reasonMessage: Record<string, string> = {
        called_slot_occupied: "呼び出し中に置ける番号は1つまでです。",
        in_progress_slots_occupied: "入場中に置ける番号は2つまでです。",
      };
      setMessage(reasonMessage[String(data?.reason)] ?? "番号を移動できませんでした。");
      return;
    }
    setTicketMenu(null);
    await refresh();
    ...
    <section className="subScreen waitingConsoleScreen" aria-labelledby="waiting-console-title">
      ...
      {!snapshot ? <p className="waitingConsoleLoading">整理券情報を読み込み中</p> : view === 1
        ...
        : <StaffBoard tickets={snapshot.tickets} now={now} busy={busy} onMove={moveTicket} onAdd={addManual}
            onOpenMenu={setTicketMenu} onReset={resetTickets} />}
      {ticketMenu && (
        <TicketActionMenu
          menu={ticketMenu}
          busy={busy}
...
function StaffBoard({ tickets, now, busy, onMove, onAdd, onOpenMenu, onReset }: {
...
function Lane({ title, status, tickets, now, onMove, onOpenMenu, hideTitle = false }: {
  ...
  return <section className={`waitingLane waitingLane-${status}`}
    ...
    <div className="waitingTicketStrip">
      ...
      {lane.map((ticket) => {
        const elapsedFrom = status === "pending" ? ticket.pending_at : status === "waiting" ? ticket.issued_at : status === "redeemed" ? ticket.redeemed_at : null;
        const elapsed = elapsedFrom ? Math.max(0, now - new Date(elapsedFrom).getTime()) : 0;
        const entryCountdown = Math.max(0, 6 * 60 * 1000 - elapsed);
        const countdown = Math.max(0, new Date(ticket.scheduled_at).getTime() - now);
        const overdue = status === "pending" && elapsed >= 900000;
        const entryExpired = status === "redeemed" && entryCountdown === 0;
        const showScheduledTime = Math.floor(now / 3000) % 2 === 1;
        return <button type="button" draggable key={ticket.ticket_number}
          className={`waitingTicket ${ticket.manually_issued ? "manual" : ""} ${overdue ? "overdue" : ""} ${entryExpired ? "entryExpired" : ""}`}
          onClick={(event) => {
          ...
          {(status === "pending" || status === "waiting" || status === "redeemed") && (
            <span className="waitingTicketTimes">
              ...
              {status === "waiting" && (
                <small className="waitingTicketCountdown">
                  {showScheduledTime ? `${formatGuideTime(ticket.scheduled_at)}に案内` : `あと ${formatElapsed(countdown)}`}
                </small>
              )}
            </span>
          )}
